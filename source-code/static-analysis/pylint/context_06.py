#!/usr/bin/env python
'''Code to illustrate creating your own context manager, Two
approaches are shown

  * creating a class that implments a context manager,
...
class ContextTest:
    ...
    def __init__(self, context_nr):
        '''Constructor that sets the initial counter value'''
        self._context_nr = context_nr
        print(f'created with {self._context_nr}')

    def __enter__(self):
        '''Method that initializes the context'''
        print(f'entering {self._context_nr}')
        return self

    def __exit__(self, exception_type, exception_value, backtrace):
        '''Method that is run upon exiting the context, either
        normally, or by an exception thrown in the body'''
...
@contextmanager
def label(name):
    '''Context manager implemented as a funciton'''
